...
export function HeaderExamSwitcher() {
  ...
  const isCSE = activeWorkspace?.examId === "cse";
  const fullName = isCSE ? "Civil Service Exam" : examConfig?.fullName || "Examination";
      ...
      <div className="relative inline-block text-left ml-1" ref={menuRef} onKeyDown={handleKeyDown}>
        {activeWorkspace ? (
          <button
            ...
            className="group inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-brand-50/80 dark:bg-brand-950/60 text-brand-800 dark:text-brand-300 border border-brand-200/80 dark:border-brand-800/80 hover:bg-brand-100/80 dark:hover:bg-brand-900/60 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-600 cursor-pointer shadow-2xs"
          >
            {/* Desktop Full Label */}
        ...
        {isOpen && (
          <div
            ...
            className="absolute left-0 mt-2 w-72 sm:w-80 rounded-2xl bg-white dark:bg-[#1E191C] border border-border shadow-xl py-2 z-50 animate-in fade-in zoom-in-95 duration-100"
          >
            ...
            <div className="border-t border-border/80 pt-1.5 mt-1 space-y-0.5">
              ...
              >
                ...
                <span>Explore all examinations</span>
              </Link>
