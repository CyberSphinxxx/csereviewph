...
export function ExamCalendarCard({
  ...
  const parsedTarget = new Date(`${config.targetDate}T00:00:00`);
  ...
  return (
    ...
    >
        ...
        <div>
          ...
          {daysUntilExam > 0 ? (
            ...
            <div className="text-xs font-semibold text-amber-700 mt-0.5">
              Target date has passed · Choose a new date
            </div>
          )}
        </div>
      ...
      {/* Mini Month Calendar for Orientation */}
      <div className="rounded-xl border border-slate-100 p-3 bg-white">
        <div className="flex items-center justify-between mb-2.5">
          ...
          <div className="flex items-center gap-1">
            <button
              type="button"
              onClick={jumpToToday}
              className="px-1.5 py-0.5 text-[10px] font-semibold text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded transition"
              title="Jump to today"
            >
              Today
            </button>
        ...
        <div className="grid grid-cols-7 gap-1 text-center text-xs">
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            const isTargetDay =
              isTargetValid &&
              checkDate.getFullYear() === parsedTarget.getFullYear() &&
              checkDate.getMonth() === parsedTarget.getMonth() &&
              checkDate.getDate() === parsedTarget.getDate();

            return (
              <div
                key={`day-${dayNum}`}
                className={`py-1 rounded-md text-[11px] font-medium relative transition flex items-center justify-center ${
                  isTargetDay
                    ? "bg-gold-500 text-slate-950 font-bold shadow-xs ring-1 ring-gold-600"
                    : isToday
                    ? "bg-brand-50 text-brand-800 font-bold border border-brand-300"
                    : "text-slate-700 hover:bg-slate-50"
                }`}
                title={
                  isTargetDay
                    ? `Target Exam: ${config.examName}`
                    : isToday
                    ? "Today"
                    : undefined
                }
      ...
      <div className="space-y-1.5 pt-1">
        <div className="flex items-center justify-between text-xs font-semibold">
          ...
          <span className="text-slate-900 font-bold">
            {dailyAnswered} / {config.dailyGoal} items
          </span>
        </div>
        <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
          <div
            className={`h-full transition-all duration-500 rounded-full ${
              dailyAnswered >= config.dailyGoal ? "bg-emerald-500" : "bg-brand-600"
            }`}
            style={{ width: `${dailyProgressPercent}%` }}
          />
        </div>
        <p className="text-[11px] text-slate-400">
          {dailyAnswered >= config.dailyGoal
            ? "✓ Daily goal completed for today!"
            : `${config.dailyGoal - dailyAnswered} more items to meet your daily target`}
        </p>
      </div>
          ...
          <form onSubmit={handleSaveEdit} className="space-y-3 text-xs">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (val === "2027-03-14") {
                    ...
                    setEditName("Custom Target Exam");
                  }
                }}
            ...
            <div>
              <label
                ...
                className="block text-[11px] font-bold text-slate-700 mb-1"
              >
                Daily Question Goal (5–200 items)
              </label>
              <input
            ...
            <div className="flex items-center gap-2 pt-1">
              <button
                type="submit"
                className="flex-1 py-2 rounded-lg bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs transition flex items-center justify-center gap-1"
              >
              ...
              <button
                ...
                className="px-3 py-2 rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-100 font-semibold text-xs transition"
              >
                Cancel
              </button>
            </div>
