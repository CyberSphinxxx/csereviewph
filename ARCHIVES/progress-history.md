...
**Current status: STANDALONE AUTH PAGES (CONCEPT C) COMPLETE (VERIFY EXIT 0)**

## Done

- **Standalone Auth Pages (Concept C from `signinmodal.html`)**:
  - New `AuthStandaloneLayout` + `AuthStandaloneForm`: split page with a fixed-maroon brand panel (owl with tracked pupils, Live chip, benefits) beside a theme-aware form card; the maroon panel is a shell choice and stays constant in light/dark while the form side follows site tokens.
  - All five auth states implemented: sign in, create account, forgot password, check-inbox (no email-existence confirmation), and new password; inline per-field validation with aria-invalid/aria-describedby, focus-first-invalid, eye toggle, live 0/8 meter, real autocomplete attributes.
  - Real better-auth wiring: `signIn.email`, `signUp.email`, `requestPasswordReset`, and `resetPassword` (newly exported from auth-client); duplicate-email signup surfaces as an inline email error.
  - Routes: `/sign-in`, `/create-account`, `/forgot-password` re-pointed; NEW `/reset-password?token=` page created (reset emails already redirected there but the page did not exist); tokenless visits render an invalid-link state.
  - Switcher and cross-links navigate real routes so URLs stay canonical; privacy notice appears only on create-account.
  - Removed obsolete `AuthPageLayout`; `AuthForm`/`AuthModal` untouched (still used by the header modal and pinned by their tests).
  - Tests: rewrote `tests/unit/components/auth-pages.test.tsx` (13 tests covering all four routes, validation, eye toggle, reset-sent/success/invalid states); updated `tests/e2e/auth-modal.spec.ts` dedicated-page assertions and added `/reset-password` coverage.
